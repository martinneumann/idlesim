...
export function check_if_boundaries_are_reached(position: position, world: World) {
    // Checks if the person is outside the boundaries of the world
    if (position.x <= 0.0 || position.x >= world.width ||
        position.y <= 0.0 || position.y >= world.height ||
        position.z <= -10.0 || position.z >= world.z_depth) {
